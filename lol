...
function OrionLib:MakeWindow(WindowConfig)
    ...
    local TitleLabel = AddThemeObject(SetProps(MakeElement("Text", "Orion Lib"), {
        Position = UDim2.new(0, 38, 0, 8),
        Size = UDim2.new(0.6, 0, 0, 24),
        TextXAlignment = "Left"
    }), "Text")

    -- Function to smoothly transition between names
    local function TransitionTitle(newName)
        local currentName = TitleLabel.Text
        ...
        for i = 1, steps do
            ...
            if ratio <= 0.5 then
                ...
                TitleLabel.TextTransparency = ratio * 2
            else
                -- Change text and fade in new name
                if TitleLabel.Text == currentName then
                    TitleLabel.Text = newName
                end
                TitleLabel.TextTransparency = 2 - (ratio * 2)
            end
        end
        TitleLabel.TextTransparency = 0
    end

    -- Function to toggle between names
    local function ToggleTitle()
        if TitleLabel.Text == "Verbal Hub V3" then
            TransitionTitle("Game Land Hub")
        else
            TransitionTitle("Verbal Hub V3")
        end
    end

    -- Set initial title
    TitleLabel.Text = "Verbal Hub V3"
    
    -- Add a button or other trigger to toggle the title
    -- For example, you could add this to your existing CloseBtn:
